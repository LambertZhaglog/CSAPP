...
bool instr_valid = f_icode in 
	{ INOP, IHALT, IRRMOVL, IIRMOVL, IRMMOVL, IMRMOVL,
	  IOPL, IJXX, ICALL, IRET, IPUSHL, IPOPL, IIADDL };# add iaddl
...
bool need_valC =
	f_icode in { IIRMOVL, IRMMOVL, IMRMOVL, IJXX, ICALL, IIADDL }; # add iaddl
...
int d_srcB = [
	D_icode in { IOPL, IRMMOVL, IMRMOVL, IIADDL  } : D_rB; # add iaddl
...
int d_dstE = [
	D_icode in { IRRMOVL, IIRMOVL, IOPL, IIADDL} : D_rB; # add iaddl
	D_icode in { IPUSHL, IPOPL, ICALL, IRET } : RESP;
...
## Forward into decode stage for valA # to handle the load / use hazard 
int d_valA = [
	D_icode in { ICALL, IJXX } : D_valP; # Use incremented PC # so the d_valA will be invalid for ICALL, IJXX
	...
	# why in order of  E M W, because in pipe line, the instrution just before this instruction is in execution stage,two step forward in memory stage, and so on. we must use the value just store
];
...
int aluA = [
	...
	E_icode in { IIRMOVL, IRMMOVL, IMRMOVL, IIADDL } : E_valC; # add iaddl
...
int aluB = [
	E_icode in { IRMMOVL, IMRMOVL, IOPL, ICALL, 
		     IPUSHL, IRET, IPOPL, IIADDL } : E_valB; # add iaddl
...
bool set_cc = E_icode in { IOPL, IIADDL} &&
	...
	!m_stat in { SADR, SINS, SHLT } && !W_stat in { SADR, SINS, SHLT }; # add iaddl
